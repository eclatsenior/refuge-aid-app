-- Fix ROUND() error in get_dashboard_kpis function by converting FLOAT to NUMERIC
...
BEGIN
  ...
  -- CALCULAR RIESGO ACTIVO ACTUAL (con cast a NUMERIC)
  risk_score_current := LEAST(
    (unresolved_alerts * 20 * 0.4) +
    CASE 
      WHEN total_employees > 0 
      THEN ((total_employees - active_employees_3d)::NUMERIC / total_employees * 100) * 0.3
      ELSE 0 
    END +
    (CASE 
  ...
  SELECT 
    CASE WHEN total_employees > 0 
      THEN (((total_employees - active_before)::NUMERIC / total_employees * 100) * 0.3 + alerts_before * 20 * 0.4) - risk_score_current
      ELSE 0
    END
  ...
  -- Construir resultado JSON con training_completion CORREGIDO y ROUND con cast explícito
  SELECT jsonb_build_object(
    'has_data', has_data,
    'active_risk', jsonb_build_object(
      'score', CASE WHEN has_data THEN ROUND(risk_score_current::NUMERIC, 1) ELSE NULL END,
      'trend_7d', CASE WHEN has_data THEN ROUND((risk_score_current - COALESCE(risk_score_7d_ago, risk_score_current))::NUMERIC, 1) ELSE NULL END,
      'trend_30d', CASE WHEN has_data THEN ROUND((risk_score_current - risk_score_30d_ago)::NUMERIC, 1) ELSE NULL END
    ),
    'incidents_today', (
      SELECT COUNT(*) 
      FROM emergency_alerts e
    ...
    'avg_mood', CASE WHEN has_data THEN ROUND(avg_mood_24h::NUMERIC, 1) ELSE NULL END,
    'checkins_count', (
      SELECT COUNT(*) 
      FROM mood_check_ins m
      INNER JOIN employee_assignments ea ON ea.employee_id = m.employee_id
    ...
    'training_completion', (
      ...
      employee_progress AS (
        SELECT 
          ea.employee_id,
          COUNT(DISTINCT vp.video_id)::NUMERIC as videos_completed
        FROM employee_assignments ea
      ...
      SELECT COALESCE(
        CASE 
          ...
          THEN ROUND(
            (((SELECT SUM(videos_completed) FROM employee_progress) / 
             (total_employees * (SELECT count FROM total_videos))) * 100)::NUMERIC,
            1
          )
          ELSE 0
        END,
